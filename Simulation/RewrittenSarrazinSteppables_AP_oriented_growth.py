...
class RegionalMitosis(MitosisSteppableBase):
   def __init__(self,_simulator,_frequency=1):
      ...
      self.r_mitosis_R2 = [0.0,0.0,0.0] #0.3 #0.0 # approximate fraction of cells dividing in a given window in region 2 (anterior GZ)
...
class CellExpansionForces(SteppableBasePy):
   def __init__(self,_simulator,_frequency):
        SteppableBasePy.__init__(self,_simulator,_frequency)
        self.nTrackerPlugin=CompuCell.getNeighborTrackerPlugin()
        self.lambdaExpand=0.5
       
   def step(self,mcs):
      nTrackerAccessor=self.nTrackerPlugin.getNeighborTrackerAccessorPtr()
      self.calculate_expansion_forces()
      for cell in self.cellList:
         self.apply_expansion_forces(cell,nTrackerAccessor)
       
   def calculate_expansion_forces(self):
      for cell in self.cellList:
         if cell: 
            delV=cell.targetVolume-cell.volume
            F=self.lambdaExpand*delV**2
            cellDict=CompuCell.getPyAttrib(cell)
            cellDict["ExpandForce"]=F
   
   def apply_expansion_forces(self,cell,nTrackerAccessor):
      U=0
      cellDict=CompuCell.getPyAttrib(cell)
      cellNeighborList=CellNeighborList(nTrackerAccessor,cell)
      for neighbor in cellNeighborList:
         if neighbor:
            neighborDict=CompuCell.getPyAttrib(neighbor)
            del_y=neighbor.yCOM-cell.yCOM
            if del_y>0:
               del_x=neighbor.xCOM-cell.xCOM
               c=math.sqrt(del_x**2+del_y**2)
               mag=(1.0/c)*neighborDict["ExpandForce"]+cellDict["ExpandForce"]
               U+=mag
      print "cell id:" + str(cell.id) + " lambdaVecY=" + str(cell.lambdaVecY) + " U=" + str(U)
      cell.lambdaVecY+=self.lambdaExpand*U
